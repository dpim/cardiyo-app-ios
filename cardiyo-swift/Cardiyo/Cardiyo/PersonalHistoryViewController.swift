...
class PersonalHistoryViewController: UITableViewController, CardCellDelegate {
    ...
    func segmentChanged(cell: CardTableCellView) {
        print("segment now set to: \(cell.segmentedControl?.selectedSegmentIndex)  for: \(cell.tag)")
    ...
    func cellPinched(cell:CardTableCellView){
        ...
        self.performSegue(withIdentifier: "HistoryToDetails", sender: cell)
    }
    ...
    func makeCellRequest(refresh: Bool){
        if AccessToken.current == nil {
            ...
            Alamofire.request("https://cyoapp.azurewebsites.net/v1/history", method: .get, headers: headers).responseJSON
                { response in
                    do {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "HistoryToDetails" {
            ...
            destinationVC.isViewersImage = true;
